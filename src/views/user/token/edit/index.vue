<script lang="ts" setup>
    ...
    const schemas = schemaInfo.value.fields
      ...
      .map((item: any) => {
        ...
        return {
          ...
          type: item.type,
          defaultValue:
            routerName === 'UserContentEdit' ? contentInfo.value[item.name] : item.defaultValue,
          componentProps,
  ...
  async function handleSubmit(values) {
    ...
    router.push({ path: `/user/${schemaId}` });
  }
  ...
  <div>
    <div class="n-layout-page-header">
      <n-card
        :bordered="false"
        :title="
          (routerName === 'UserContentCreate' ? '创建' : '更新') +
          '【' +
